...
<%--容器放好--%>
<table id="dgSFs"></table>
<%--通过js代码来渲染容器--%>
<script>
    //各个按钮的JS动作
    function add() {
        ttshop.addTabs('新增体育设施', 'sf_add');
    }
    //点击编辑按钮动作
    function editor(){
        var row = $('#dgSFs').datagrid('getSelected');
        if (row.length == 0) {
    ...
    function remove() {
        ...
        //取到客户选中的记录集合
        var rows = $('#dgSFs').datagrid('getSelections');
        if (rows.length == 0) {
            $.messager.alert('警告', '请选中至少一条记录！', 'warning');
            return;
        }
    ...
    $('#dgSFs').datagrid({
        ...
        columns: [[
            ...
            {field: 'category', title: '类别', sortable: true},
            {field: 'name', title: '名称'},
            {
                field: 'damage', title: '状态', width: 100, formatter: function (value, row, index) {
               if(value=="Y"){
                   return "损坏";
               }else if(value=="N"){
                   return "正常";
